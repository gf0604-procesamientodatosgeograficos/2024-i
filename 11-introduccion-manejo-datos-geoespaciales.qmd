...
En el siguiente bloque de código, se utiliza `st_read()` para leer un [archivo CSV con registros de presencia de félidos de Costa Rica](datos/gbif/felidos.csv), el cual contiene dos columnas llamadas `decimalLongitude` y `decimalLatitude` correspondientes a la longitud decimal y latitud decimal en las que fue observado cada felino. Este archivo proviene de una [consulta al portal de datos de la Infraestructura Mundial de Información en Biodiversidad (GBIF)](https://doi.org/10.15468/dl.gmgzh6).
...
El siguiente bloque de código muestra las capas de provincias (polígonos) y de registros de presencia de félidos (puntos) sobre una capa base de [OpenStreetMap (OSM)](https://www.openstreetmap.org/) y otra de ESRI.
...
2.  Agrupe puntos agrupados (*clusters*) correspondientes a los registros de presencia mediante la función `markerClusterOptions()` (vea [Leaflet for R - Markers](https://rstudio.github.io/leaflet/markers.html)).
...
#### leaflet
El método [addRasterImage()](https://rdrr.io/pkg/leaflet/man/addRasterImage.html) de `leaflet` despliega un objeto `SpatRaster` en un mapa. En el siguiente bloque, se muestran en un mapa leaflet las capas de altitud (raster), provincias (polígonos) y registros de presencia de félidos (puntos) sobre una capa base de [OpenStreetMap (OSM)](https://www.openstreetmap.org/) y otra de ESRI.
